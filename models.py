...
#create Shallow Decoder Network (SDN)
...
#Training SHRED and SDN
def fit(model, train_dataset, valid_dataset, criterion='MSE', batch_size=64, num_epochs=4000, lr=1e-3, verbose=False, patience=5):
    '''Function for training SHRED and SDN models'''
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size)
    if criterion=='MSE':
        criterion = torch.nn.MSELoss()
    else:
        #assume turbulence loss
        criterion = custom_turbulence_loss
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 20 == 0 or epoch == 1:
            ...
            #NOTE: Patience parameter used for early stopping
            #when the loss function is below threshold, patience kicks in to train only patience=5 more epochs
...
def custom_turbulence_loss(output, target, lambda_spectrum=0.1):
    ...
    # Compute FFT of output and target (assuming last spatial dim is for turbulence)
    output_fft = torch.fft.rfft(output, dim=-1)
    target_fft = torch.fft.rfft(target, dim=-1)

    # Compute energy spectrum (squared magnitude of FFT)
    output_spectrum = torch.abs(output_fft) ** 2
    target_spectrum = torch.abs(target_fft) ** 2

    # Compute L2 distance between spectra (you could also use log space or relative error)
    spectral_loss = torch.nn.functional.mse_loss(output_spectrum, target_spectrum)

    total_loss = mse_loss + lambda_spectrum * spectral_loss
    return total_loss